...
export class ItemsList {
  ...
  constructor(private _ngSelect: NgSelectComponent, private _selectionModel: SelectionModel) {}
  ...
  setItems(items: any[]) {
    ...
    if (this._ngSelect.hideDisabled) {
      this._filteredItems = [...this._items.filter((x) => !x.disabled)];
    } else {
  ...
  filter(term: string): void {
    ...
    const hideDisabled = this._ngSelect.hideDisabled;
    ...
    for (const key of Array.from(this._groups.keys())) {
      ...
      for (const item of this._groups.get(key)) {
        ...
        if (hideDisabled && ((item.parent && item.parent.disabled) || item.disabled)) {
          continue;
        }
